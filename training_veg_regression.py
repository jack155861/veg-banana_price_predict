...
# shape (samples, 7 七個時間段, 7 每個時間段往前推七個時間點, 1 每個時間點只有一個維度, 7 七種特徵值)
...
def model_main_2(train_X, val_X,
    ...
    if folder not in file_exit_:
        ...
        #train_Y_1 = Y_train[[x for x in Y_train.columns if '漲跌狀態' in x]].values#.astype(int)
        #train_Y_1 = [train_Y_1[:,x] for x in range(train_Y_1.shape[1])]
        #val_Y_1 = Y_val[[x for x in Y_val.columns if '漲跌狀態' in x]].values#.astype(int)
        ...
        #train_Y_1 = Y_train[[x for x in Y_train.columns if '漲跌狀態' in x]].values
        #val_Y_1 = Y_val[[x for x in Y_val.columns if '漲跌狀態' in x]].values
        ...
        #class_weight_ = [class_weight_cal(x, banana_Y_train) for x in [x for x in banana_Y_train.columns if '漲跌狀態' in x]]
        ...
        # RepeatVector - sparse_categorical_crossentropy
        #class_weight_ = [class_weight_cal(x, banana_Y_train) for x in [x for x in Y_train.columns if '漲跌狀態' in x]]
        #class_weight_ = {k:v for e in class_weight_ for (k,v) in e.items()}
        #class_weight_ = np.array([max([class_weight_[x][y] for x in class_weight_.keys()]) for y in [0,1,2]])
